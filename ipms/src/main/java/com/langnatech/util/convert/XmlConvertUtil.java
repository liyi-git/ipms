...
import com.langnatech.util.AssertUtil;
import com.langnatech.util.ExceptionUtil;
import com.langnatech.util.Reflections;
/**
 * 使用Jaxb2.0实现XML<->Java Object的转换. 在创建时需要设定所有需要序列化的Root对象的Class. 特别支持Root对象是Collection的情形.
 */
public class XmlConvertUtil {
    ...
    public static <T> T fromXml(File xmlFile, Class<T> clazz) {
        try {
            return (T)createUnmarshaller(clazz).unmarshal(xmlFile);
        } catch (JAXBException e) {
            throw ExceptionUtil.unchecked(e);
        }
    }
    ...
    public static <T> T fromXml(InputStream inputStream, Class<T> clazz) {
        try {
            return (T)createUnmarshaller(clazz).unmarshal(inputStream);
        } catch (JAXBException e) {
            throw ExceptionUtil.unchecked(e);
        }
    }
