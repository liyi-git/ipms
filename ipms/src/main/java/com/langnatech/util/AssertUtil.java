 ...
 * clearly at runtime. <p>For example, if the contract of a public method states it does not allow {@code null}
 * arguments, Assert can be used to validate that contract. Doing this clearly indicates a contract violation when it
 * occurs and protects the class's invariants. <p>Typically used to validate method arguments rather than configuration
 * properties, to check for cases that are usually programmer errors rather than configuration errors. In contrast to
 * config initialization code, there is usally no point in falling back to defaults in such methods. <p>This class is
 * similar to JUnit's assertion library. If an argument value is deemed invalid, an {@link IllegalArgumentException} is
 * thrown (typically). For example: <pre class="code"> Assert.notNull(clazz, "The class must not be null");
 * Assert.isTrue(i > 0, "The value must be greater than zero");</pre> Mainly for internal use within the framework;
 * consider Jakarta's Commons Lang >= 2.0 for a more comprehensive suite of assertion utilities.
 * 
 * @author Keith Donald
...
public abstract class AssertUtil {
     ...
     * least one non-whitespace character. <pre class="code">Assert.hasText(name, "'name' must not be empty");</pre>
     * 
     * @param text the String to check
     * @param message the exception message to use if the assertion fails
    ...
    /**
     * Assert that {@code superType.isAssignableFrom(subType)} is {@code true}. <pre
     ...
     * isTrue if you wish to throw IllegalArgumentException on an assertion failure. <pre class="code">Assert.state(id
     * == null, "The id property must not already be initialized");</pre>
     * 
     * @param expression a boolean expression
     * @param message the exception message to use if the assertion fails
