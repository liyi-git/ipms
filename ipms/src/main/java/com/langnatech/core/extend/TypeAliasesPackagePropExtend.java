package com.langnatech.core.extend;
...
/**
 * 该类实现了FactoryBean接口，在applicationContext-dao-base.xml文件中配置
...
public class TypeAliasesPackagePropExtend implements FactoryBean<String> {
	private final Logger log = LoggerFactory
			.getLogger(TypeAliasesPackagePropExtend.class);
	...
	public String getObject() throws Exception {
		...
		try {
			...
			String result[] = packagePathList
					.toArray(new String[packagePathList.size()]);
			String resultStr = StringUtils.arrayToDelimitedString(result, ",");
			LoggerUtil.info(log, "Mybatis scan typeAliasesPackage is:[{}]",
					resultStr);
			return resultStr;
		} catch (IOException e) {
			log.error("Mybatis scan typeAliasesPackage error:[{}]",
					e.getMessage());
		}
		return null;
	}
