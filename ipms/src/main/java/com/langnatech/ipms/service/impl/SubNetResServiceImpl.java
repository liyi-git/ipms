...
public class SubNetResServiceImpl implements SubNetResService {
    ...
    public boolean splitSubnet(SubNetResEntity[] entitys) {
        ...
        for (int i = 0; i < entitys.length; i++ ) {
            IPPoolConfEntity poolEntity = this.ipPoolConfDao.selectPoolConfByPoolId(entitys[i].getPoolId());
            SubNetResEntity entity = new SubNetResEntity();
    ...
    public void insertBatchIp(String beginIp,String endIp,String subnetId){
		...
		try {
             ...
             System.out.println("begin:"+beginAddr+"  endIp:"+endAddr);
			...
			for (IPAddressResEntity ipEntity : ipList) {
				sqlsession.insert("com.langnatech.ipms.dao.IPAddressResDao.insert", ipEntity);
			}
			sqlsession.commit();
		} catch (Exception e) {
			e.printStackTrace();
